...
def generate_txt_report():
    ...
    txt_filename = os.path.join("InvitationOutput",first_name + '-' + last_name + "-Invitation_report.txt")
    doc = SimpleDocTemplate(txt_filename, pagesize=letter, rightMargin=12, leftMargin=12, topMargin=12, bottomMargin=6)
    document = []
...
def open_invitation_report_window(homepage_window, root):
    ...
    btn_report.pack(pady=20, side = tk.LEFT)

        #button to generate pdf report
    btn_report = ttk.Button(invitation_report_window, text="Generate TXT Report", command=generate_txt_report)
    btn_report.pack(pady=20, side = tk.RIGHT)
